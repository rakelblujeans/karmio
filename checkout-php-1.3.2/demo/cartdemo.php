...
//   UseCase3();
 ...
 // CarrierCalcUsecase();
  ...
  function Usecase($name,$desc,$price,$pId,$userId,$donated,$quantity,$expiry_date) {
      ...
      $currency);
      $total_count = 12;
	  $invoice= substr($pId,0,2) . substr(strtoupper(mt_rand()),0,2) . substr(time(),3,2);
      
      $item_1 = new GoogleItem("$name",      // Item name
      ...
      $cart->SetEditCartUrl("http://www.karm.io/design");
	  $cart->SetMerchantPrivateData(
              new MerchantPrivateData(array("pId" => $pId,"userId"=>$userId,"paid"=>$price,"donated"=>$donated,"quantity"=>$quantity,"expiry_date"=>$expiry_date,'invoice'=>$invoice)));

		
      // Specify "Return to xyz" link
      $cart->SetContinueShoppingUrl("http://www.karm.io/design");
  ...
  function UseCase3() {
   ...
   $merchant_id = "231472301732456";  // Your Merchant ID
    ...
    $cart->SetMerchantCalculations(
        "http://www.karm.io/design/checkout-php-1.3.2/demo/responsehandlerdemo.php", // merchant-calculations-url
