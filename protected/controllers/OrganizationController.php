...
// TODO: no idea what this represents. It's not the charity, product, or seller.
...
class OrganizationController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Organization']))
		{
			...
			$data = User::model()->find("email = '$model->email'");
			$uid=$data->id;
			...
			if($model->save())
			{
                ...
                $cm->newNonprofitAdmin($model);

				$command = Yii::app()->db->createCommand();
				$command->update('tbl_user', array(
  						  'password'=>md5($password),'organization_id'=>$model->primaryKey ,), 'id=:id', array(':id'=>$uid));
						  
				$role = new UserRole;
				$role->user_id = $uid;
				$role->value = 'nonprofitadmin';
				$role->save(false);
