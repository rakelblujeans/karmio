...
class Thumbnail
	 ...
	 * @return 
	 */
	function Thumbnail($resource_file, $max_width, $max_height, $destination_file="", $compression=80, $transform="", $aspect_ratio=true, $crop=false)
	{
	...
	{	
		...
		if($this->s !== Null)
			...
			if($this->crop == true)
			{
				$ratio = (($this->i / $this->j) < ($this->m / $this->n)) ?($this->m /$this->i) : ($this->n / $this->j);
				$x = max(0, round($this->i/2 - ($this->m/2)/$ratio));
			    $y = max(0, round($this->j / 2 - ($this->n / 2) / $ratio));
				$this->u = imagecopyresampled($this->t, $this->s, 0, 0, $x, $y, $this->q, $this->r, $this->i, $this->j);
			}
			else
			{
				//echo $this->q.','.$this->r.'='.$this->i.','.$this->j.'<br>';
	...
	public static function resize_crop($src, $dest, $required_width, $required_height)
	{
		//echo $required_width;
		//echo $required_height;
		...
		$source_path = $src; 
		  ...
		  $desired_aspect_ratio = $required_width / $required_height;
		
		  if ( $source_aspect_ratio > $desired_aspect_ratio )
		  {
			$temp_height = $required_height;
			$temp_width = ( int ) ( $required_height * $source_aspect_ratio );
		  }
		  else
		  {
			
			$temp_width = $required_width;
			$temp_height = ( int ) ( $required_width / $source_aspect_ratio );
		  }
		...
		$x0 = ( $temp_width - $required_width ) / 2; 
		$y0 = ( $temp_height - $required_height ) / 2;
		
		  $desired_gdim = imagecreatetruecolor( $required_width, $required_height );
		  imagecopy(
			$desired_gdim,
			$temp_gdim,
			0, 0,
	...
	public static function thumbnail_box($img, $box_w, $box_h) {
